describe('Navigation', () => {
    it('Visits the homepage and navigates to the calendar page', () => {
      ...
      cy.get('#loginName').type('jp@jp.fr');
      ...
      cy.wait(1000);
      cy.contains('Norauto').click({force:true}); 
      cy.wait(1000);
      cy.contains('Vidange').click({force:true});
      cy.wait(1000);
      cy.contains('Réparation carrosserie').click({force:true});
      cy.wait(1000);
      cy.get('#TestButtonCY').click({force:true});
      cy.wait(1000);
      cy.contains('Valider').click({force:true});
      cy.visit('http://localhost:3000/calendrier');
      cy.contains('jp@jp.fr').click({force :true});
